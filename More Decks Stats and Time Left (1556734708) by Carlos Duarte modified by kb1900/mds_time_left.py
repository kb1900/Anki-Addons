...
def renderStats(self, _old):
    ...
    buf = "<div style='display:table;padding-top:1.5em;'>" \
        ...
        + _old(self) + "<hr>" \
        + _("New Cards") \
        + ": &nbsp; <font color=#4297F9> %(d)s</font>" % dict(d=new) \
        + " &nbsp; " + _("Learn") \
        + ": &nbsp; <font color=#00FFFF>%(c)s</font>" % dict(c=lrn) \
        + " &nbsp; <span style='white-space:nowrap;'>" + _("To Review") \
        + ": &nbsp; <font color=#00FF7F>%(c)s</font>" % dict(c=due) \
        + "</span>" \
        ...
        + "<br> " + _("%.01f cards / hour") % (speed*60) \
        + "</span><br><br>" + _("%.01f cards / minute") % (speed) \
        + "</span><br><br>" \
        + _("TTC:") + "&nbsp;" + ngettext(
